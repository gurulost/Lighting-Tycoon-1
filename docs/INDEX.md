# Documentation Index

## Core Docs

- `../README.md` - setup and scripts
- `architecture.md` - system architecture overview
- `development.md` - local development guide
- `configuration.md` - environment variables
- `game_systems.md` - system overview
- `tutorial.md` - tutorial and first-session spec
- `content_pipeline.md` - how orders are generated
- `tuning.md` - balance knobs
- `drop_tables.md` - supplier drop tables and rates

## Engineering & Operations

- `api.md` - REST API reference
- `database.md` - database schema and migrations
- `testing.md` - testing guidance
- `deployment.md` - build and deployment steps
- `operations.md` - production runbook notes
- `adr/README.md` - architecture decision records
- `../.env.example` - environment variable template (export in shell)

## Narrative & UX

- `narrative.md` - story rules and beats
- `glossary.md` - player-facing definitions
- `audio.md` - SFX map
- `../design_guidelines.md` - visual and UI direction

## QA & Production

- `qa.md` - QA harness
- `release_checklist.md` - production release checklist
- `phase-1-2-3-tier-16-production-checklist.md` - source-of-truth rollout checklist for explicit phases and tier cap expansion to 16
- `telemetry.md` - analytics runbook and event catalog
- `changelog.md` - change log
- `game_audit_report.md` - current state audit

## Art Sources

- `art_source/README.md` - source assets and export sizes

## Policy & Governance

- `../CONTRIBUTING.md` - contribution guide
- `../CODE_OF_CONDUCT.md` - community standards
- `../SECURITY.md` - vulnerability reporting
- `../SUPPORT.md` - support and help
- `../LICENSE.md` - licensing
- `../CHANGELOG.md` - engineering changelog

## Other

- `../replit.md` - replit-specific overview and notes
